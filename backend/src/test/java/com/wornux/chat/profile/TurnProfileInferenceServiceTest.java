...
class TurnProfileInferenceServiceTest {
  ...
  private final TurnProfileInferenceService service =
      new TurnProfileInferenceService(new QuestionAnswerProfileSignalService());
  ...
  void infer_detects_topics_examples_and_misconceptions() {
    var update =
        service.infer(
            UUID.randomUUID(),
            UUID.randomUUID(),
            "No entiendo la diferencia entre contador y acumulador en un for, dame un ejemplo paso"
                + " a paso",
        ...
        service.infer(
            ...
            List.of(
                new StoredChatMessage(
                    MessageType.USER, "why does my array index start at 0", Instant.now())),
            List.of(),
            List.of());
  ...
  void infer_merges_interactive_question_signals() {
    ...
    var interaction =
        new QuestionInteractionService.CompletedQuestionInteraction(
            routing,
