...
class ProfileAwareResponseAdvisorTest {
  ...
  private final ProfileAwareResponseAdvisor advisor =
      new ProfileAwareResponseAdvisor(150, studentProfileService, profileProperties);
  ...
  void injects_profile_system_message_when_enabled() {
    ...
    assertThat(patched.prompt().getInstructions().getLast().getText())
        .contains("Priority weak topics: [loops]");
    assertThat(patched.context())
        .containsEntry(ProfileAwareResponseAdvisor.PROFILE_VERSION_CONTEXT_KEY, 3L);
  }
  ...
  void leaves_request_unchanged_in_shadow_mode() {
    ...
    assertThat(patched.context())
        .containsEntry(ProfileAwareResponseAdvisor.PROFILE_VERSION_CONTEXT_KEY, 0L);
  }
