...
class PedagogicalRoutingAdvisorTest {
  ...
  private final PedagogicalRoutingAdvisor advisor =
      new PedagogicalRoutingAdvisor(175, routingService, promptResources);

  @Test
  void direct_reference_mode_adds_targeted_examples() {
    when(routingService.classify("No recuerdo como hacer un for"))
        .thenReturn(PedagogicalRoutingMode.DIRECT_REFERENCE);
