...
class TutorGuardAdvisorTest {
    ...
    void impersonation_rule_wins_and_uses_specific_mode() {
        ...
        assertEvaluationPass(exactMatchEvaluator.evaluate(new EvaluationRequest(
                userText,
                List.of(new Document("expected=IMPERSONATION")),
                guardDecision.name()
        )));

        assertEvaluationPass(policyModeEvaluator.evaluate(new EvaluationRequest(
                userText,
                List.of(
    ...
    void out_of_scope_mode_sets_boundary_and_offer() {
        ...
        assertEvaluationPass(exactMatchEvaluator.evaluate(new EvaluationRequest(
                userText,
                List.of(new Document("expected=OUT_OF_SCOPE")),
                guardDecision.name()
        )));
    ...
    void classifier_failure_defaults_to_not_safe() {
        ...
        assertEvaluationPass(exactMatchEvaluator.evaluate(new EvaluationRequest(
                userText,
                List.of(new Document("expected=NOT_SAFE")),
                guardDecision.name()
        )));
    }
    ...
    void safe_classifier_result_keeps_request_unchanged() {
        ...
        var guardedRequest = advisor.applySafetyPolicy(buildRequest(userText), userText, advisor.ruleDecisionFor(userText));
    ...
    private static String describeGuardedRequest(ChatClientRequest request) {
        ...
        String systemMessage = request.prompt().getInstructions().stream()
    ...
    private static void assertEvaluationPass(EvaluationResponse response) {
        assertThat(response.isPass())
                .withFailMessage(response.getFeedback())
                .isTrue();
    }
    ...
    private static final class ExactMatchEvaluator implements Evaluator {
        ...
        public EvaluationResponse evaluate(EvaluationRequest evaluationRequest) {
            ...
            return new EvaluationResponse(pass, pass ? "exact match" : "expected %s but got %s".formatted(expected, evaluationRequest.getResponseContent()), Map.of());
        }
    }
    ...
    private static final class PolicyModeEvaluator implements Evaluator {
        ...
        public EvaluationResponse evaluate(EvaluationRequest evaluationRequest) {
            ...
            List<String> requiredSnippets = evaluationRequest.getDataList().stream()
            ...
            return new EvaluationResponse(pass,
                    pass ? "policy mode matches" : "response did not match expected policy mode or required snippets",
