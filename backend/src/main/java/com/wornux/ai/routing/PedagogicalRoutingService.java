...
public class PedagogicalRoutingService {
  ...
  private static final Pattern DEBUG_ATTEMPT_PATTERN =
      Pattern.compile(
  ...
  private static final Pattern EXERCISE_GUIDANCE_PATTERN =
      Pattern.compile(
  ...
  private static final Pattern DIRECT_REFERENCE_PATTERN =
      Pattern.compile(
  ...
  private static final Pattern CONCEPT_EXPLANATION_PATTERN =
      Pattern.compile(
  ...
  public PedagogicalRoutingMode classify(String userMessage) {
    ...
    return heuristicDecisionFor(normalized).orElseGet(() -> classifyWithModel(normalized));
  }
