...
import com.wornux.chat.questions.*;
...
public class StudentQuestionPanel extends Composite<Div> {
  ...
  private final Map<String, ValueSignal<Set<String>>> selectedOptionsByQuestion =
      new LinkedHashMap<>();
  ...
  private final Map<String, Map<String, VerticalLayout>> optionRowsByQuestion =
      new LinkedHashMap<>();
  ...
  private Component buildOptionButton(StudentQuestion question, StudentQuestionOption option) {
    ...
    popoverDescription.addClassNames(
        "chat-question-option-description", "chat-question-option-description-popover");
  ...
  private void bindOptionSelection(
      String questionId, String optionLabel, Button button, VerticalLayout row) {
  ...
  private static @NonNull VerticalLayout getOptionCopy(StudentQuestionOption option) {
    ...
    inlineDescription.addClassNames(
        "chat-question-option-description", "chat-question-option-description-inline");
  ...
  private void updateSubmitEnabled() {
    ...
    submitButton.setVisible(
        hasQuestionSet && activeQuestionIndex == questionSet.questions().size() - 1);
  ...
  private void updateNavigationState() {
    ...
    nextButton.setEnabled(
        hasQuestionSet && !submitting && activeQuestionIndex < questionSet.questions().size() - 1);
  ...
  Button optionButton(String questionId, String optionLabel) {
    return optionButtonsByQuestion.getOrDefault(questionId, Map.of()).get(optionLabel);
  }

  TextArea customTextArea(String questionId) {
    return customTextByQuestion.get(questionId);
  }
  ...
  boolean isOptionSelected(String questionId, String optionLabel) {
    return selectedOptions(questionId).contains(optionLabel);
  }

  boolean isOptionRowSelected(String questionId, String optionLabel) {
    var row = optionRowsByQuestion.getOrDefault(questionId, Map.of()).get(optionLabel);
    return row != null && row.getElement().getClassList().contains("is-selected");
  }

  boolean isOptionButtonSelected(String questionId, String optionLabel) {
    var button = optionButton(questionId, optionLabel);
    return button != null && "true".equals(button.getElement().getAttribute("aria-pressed"));
  }
