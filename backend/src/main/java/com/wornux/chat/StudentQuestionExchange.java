...
class StudentQuestionExchange implements Serializable {
  ...
  StudentQuestionExchange(ChatUiState state) {
    this.state = state;
  }
    ...
    synchronized (this) {
      if (pendingResponse != null && !pendingResponse.isDone()) {
        throw new IllegalStateException(
            "There is already a pending interactive question flow");
      }
  ...
  boolean submit(StudentQuestionResponse response) {
    CompletableFuture<StudentQuestionResponse> responseFuture;
    synchronized (this) {
