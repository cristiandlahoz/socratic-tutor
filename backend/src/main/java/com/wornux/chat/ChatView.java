...
public class ChatView extends Composite<Div> implements BeforeEnterObserver {
  ...
  public ChatView(ChatUiState state, ChatUiController controller, ChatProperties chatProperties) {
    ...
    Signal.effect(
        questionPanel,
        () -> questionPanel.setSubmitting(state.questionSubmissionInProgress().get()));
  ...
  private Div createInputShell(ChatUiState state) {
    ...
    Signal.effect(
        questionPanel, () -> questionPanel.setVisible(state.questionPanelVisible().get()));
  ...
  private Div createUsageBadge(ChatUiState state) {
    ...
    var helpCopy =
        new Paragraph(
            "Muestra los prompt tokens del transcript activo y el porcentaje usado contra el umbral"
                + " de compactación configurado para resumir la conversación antes de perder"
                + " calidad.");
          ...
          if (compacted && level != null) {
            var sourceLabel =
                sourceTranscriptId == null
                    ? ""
                    : " · desde %s".formatted(shortId(sourceTranscriptId));
  ...
  private CodeMessageListItem toMessageListItem(MessageVm message) {
    ...
    var item =
        new CodeMessageListItem(
            message.content(), message.createdAt(), isUserMessage ? "You" : "Socratic Tutor");
