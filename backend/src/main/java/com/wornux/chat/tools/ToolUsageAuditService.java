...
public class ToolUsageAuditService {
    ...
    public ToolUsageAuditService(MeterRegistry meterRegistry, ObservationRegistry observationRegistry) {
        this.meterRegistry = meterRegistry;
        this.observationRegistry = observationRegistry;
    }

    public <T> T audit(String toolName,
        ...
        try (Observation.Scope ignored = observation.openScope()) {
            ...
            var audit = new ToolExecutionAudit(
                    ids.conversationId(),
                    ids.clientId(),
                    ids.turnId(),
                    toolName,
            ...
            Timer.builder("tool.latency").tag("tool.name", toolName).register(meterRegistry).record(audit.latencyMs(), java.util.concurrent.TimeUnit.MILLISECONDS);
            log.info("tool_execution tool.name={} tool.status={} tool.latency_ms={} client_id={} conversation_id={} turn_id={} model_requested_tool={} profile_snapshot_version={} input_summary={} output_summary={} useful_for_profile={} failure_code={}",
                    audit.toolName(), audit.status(), audit.latencyMs(), audit.clientId(), audit.conversationId(), audit.turnId(),
                    audit.modelRequested(), audit.profileSnapshotVersion(), audit.inputSummary(), audit.outputSummary(),
                    audit.usefulForProfile(), audit.failureCode());
            return result.value();
        }
            ...
            Timer.builder("tool.latency").tag("tool.name", toolName).register(meterRegistry).record(audit.latencyMs(), java.util.concurrent.TimeUnit.MILLISECONDS);
            log.warn("tool_execution tool.name={} tool.status={} tool.latency_ms={} client_id={} conversation_id={} turn_id={} model_requested_tool={} profile_snapshot_version={} input_summary={} output_summary={} useful_for_profile={} failure_code={}",
                    audit.toolName(), audit.status(), audit.latencyMs(), audit.clientId(), audit.conversationId(), audit.turnId(),
                    audit.modelRequested(), audit.profileSnapshotVersion(), audit.inputSummary(), audit.outputSummary(),
                    audit.usefulForProfile(), audit.failureCode());
            throw exception;
