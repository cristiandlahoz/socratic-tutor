...
public class StudentProfileService {
    ...
    public StudentProfileService(StudentProfileJpaRepository profileRepository,
    ...
    public StudentProfileSnapshot load(UUID clientId) {
        ...
        var profile = profileRepository.findById(clientId).orElseGet(() -> profileRepository.save(StudentProfileEntity.create(clientId)));
        resolveStaleMisconceptions(clientId);

        var weakTopics = topicMasteryRepository.findById_ClientId(clientId).stream()
                .filter(topic -> topic.getMasteryLevel() == MasteryLevel.STRUGGLING || topic.getMasteryLevel() == MasteryLevel.DEVELOPING)
                .sorted(Comparator.comparing(StudentTopicMasteryEntity::getMasteryLevel).thenComparing(StudentTopicMasteryEntity::getEvidenceCount).reversed())
                .map(StudentTopicMasteryEntity::topicKey)
                .limit(2)
                .toList();
        var activeMisconceptions = misconceptionRepository.findByClientIdOrderByLastSeenAtDesc(clientId).stream()
    ...
    public void applyTurnSignals(UUID clientId, TurnProfileUpdate update) {
        ...
        if (update.preferredLanguage() != null && !update.preferredLanguage().isBlank()
        ...
        if (update.recommendedHelpMode() != null && profile.getHelpMode() != update.recommendedHelpMode()
                && update.toolEvidence().stream().filter(TurnProfileUpdate.ToolEvidence::useful).count() >= 2) {
        ...
        if (update.confidenceDelta().signum() != 0) {
            ...
            var clamped = nextConfidence.max(BigDecimal.ZERO).min(BigDecimal.ONE).setScale(3, RoundingMode.HALF_UP);
        ...
        for (var topic : update.topicsDetected()) {
            var mastery = topicMasteryRepository.findById(new StudentTopicMasteryId(clientId, topic))
        ...
        for (var misconceptionObservation : update.misconceptionsObserved()) {
            var misconception = misconceptionRepository.findByClientIdAndMisconceptionKey(clientId, misconceptionObservation.misconceptionKey())
                    .orElseGet(() -> StudentMisconceptionEntity.create(
                            clientId,
        ...
        signalRepository.save(StudentProfileSignalEntity.from(
                clientId,
    ...
    private void resolveStaleMisconceptions(UUID clientId) {
        ...
        for (var misconception : misconceptionRepository.findByClientIdAndStatusNotAndLastSeenAtBefore(clientId, MisconceptionStatus.RESOLVED, cutoff)) {
            misconception.resolve();
            misconceptionRepository.save(misconception);
        }
    }

    private void updateMastery(StudentTopicMasteryEntity mastery, List<TurnProfileUpdate.LevelSignal> levelSignals) {
        var matchingSignals = levelSignals.stream()
                .filter(signal -> signal.topic() == mastery.topicKey())
                .toList();
        if (matchingSignals.isEmpty()) {
            return;
        }

        long downSignals = matchingSignals.stream().filter(signal -> signal.direction() == TurnProfileUpdate.SignalDirection.DOWN).count();
        long upSignals = matchingSignals.stream().filter(signal -> signal.direction() == TurnProfileUpdate.SignalDirection.UP).count();
    ...
    private void recalculateOverallLevel(UUID clientId, StudentProfileEntity profile) {
        var levels = topicMasteryRepository.findById_ClientId(clientId).stream()
