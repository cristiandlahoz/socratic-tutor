...
import com.wornux.chat.BrowserClientService;
import com.wornux.chat.ChatUiState;
import com.wornux.chat.ConversationService;
import com.wornux.documentingest.ApproveDocumentCommand;
import com.wornux.documentingest.DocumentIngestionService;
import com.wornux.documentingest.DocumentReviewVm;
import com.wornux.documentingest.EditableSegmentVm;
import com.wornux.documentingest.StartIngestionCommand;
...
public class DocumentIngestionUiController implements Serializable {
  ...
  private final DocumentIngestionUiState state;
  ...
  public DocumentIngestionUiState state() {
    return state;
  }

  public void initializeFromRoute(String documentIdParam) {
    ensureClientContext();
    Optional<DocumentReviewVm> review =
        parseUuid(documentIdParam)
            .flatMap(
                documentId ->
                    documentIngestionService.loadReview(chatUiState.clientId().peek(), documentId));
    if (review.isEmpty()) {
                ...
                () ->
                    documentIngestionService.startIngestion(
                        new StartIngestionCommand(
                            chatUiState.clientId().peek(), fileName, mimeType, content)))
            .subscribeOn(Schedulers.boundedElastic())
            .subscribe(
                review -> runUi(ui, () -> applyReview(review)),
                error ->
                    runUi(ui, () -> state.markFailure(message(error), lastUploadedFile != null)));
  }
  ...
  public void retry(UI ui) {
    ...
    uploadPdf(
        lastUploadedFile.fileName(), lastUploadedFile.mimeType(), lastUploadedFile.content(), ui);
  }
  ...
  public void updateSegment(UUID segmentId, String content) {
    List<EditableSegmentVm> nextSegments =
        state.segments().peek().stream()
  ...
  private void applyReview(DocumentReviewVm review) {
    state.apply(review);
  ...
  private void synchronizeAddressBar(UUID documentId) {
    UI.getCurrent()
        ...
        .replaceState(
            null,
            new Location(
                "documents", QueryParameters.of(DOCUMENT_QUERY_PARAMETER, documentId.toString())));
  }
  ...
  private void ensureClientContext() {
    ...
    chatUiState.replaceConversationHistory(
        conversationService.listConversations(chatUiState.clientId().peek()));
  }
