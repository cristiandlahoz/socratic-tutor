...
public class DocumentCatalogService {
  ...
  public CatalogAnalysis analyzeOrFallback(String filename, String markdown) {
    try {
      ...
      if (catalog == null) {
        throw new DocumentIngestionException("Document catalog model returned an empty result.");
      }
      var normalized =
          catalog.normalized(
              properties.getCatalog().getMaxTags(), properties.getCatalog().getMaxQuestionExamples());
      log.info(
      ...
      var fallback =
          DocumentCatalogEntry.fallback(filename, markdown)
              .normalized(
                  properties.getCatalog().getMaxTags(),
                  properties.getCatalog().getMaxQuestionExamples());
