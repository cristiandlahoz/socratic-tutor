package com.wornux.presentation;
...
public class MainLayout extends AppLayout {
  ...
  private record TimelineThreadEntry(ConversationSummary conversation, String nodeId)
  ...
  public MainLayout(ChatUiState state, ChatUiController controller) {
    ...
    var appDescription =
        new Paragraph(
            "Tutor para explorar ideas, resolver dudas y aprender con preguntas guiadas"
    ...
    var historyDescription =
        new Paragraph("Hilos recientes ordenados por fecha y conectados como una sola ruta.");
    ...
    var emptyDescription =
        new Paragraph("Inicia un nuevo chat para empezar a construir tu historial del tutor.");
  ...
  private RouterLink createNavigationButton() {
    var iconComponent = new Icon(VaadinIcon.UPLOAD_ALT);
    iconComponent.addClassName("chat-sidebar-action-icon");

    var text = new Span("Ingestar PDF");
    var content = new Span(iconComponent, text);
    content.addClassName("chat-sidebar-action-link-content");

    var link = new RouterLink();
    link.setRoute(DocumentIngestionView.class);
    link.addClassName("chat-sidebar-action-link");
    link.getElement().setAttribute("aria-label", "Ingestar PDF");
    link.add(content);
    return link;
  }
  ...
  private Component createTimelineEntry(
      ...
      ChatUiController controller) {
    return switch (entry) {
      case TimelineDividerEntry(String _, String _) ->
          createDividerEntry((TimelineDividerEntry) entry);
      case TimelineThreadEntry(_, _) ->
          createThreadEntry(
              (TimelineThreadEntry) entry, activeConversationId, disabled, controller);
    };
  }
  ...
  private List<TimelineEntry> buildTimelineEntries(List<ConversationSummary> conversations) {
    ...
    for (var conversation : conversations) {
      groupedConversations
          .computeIfAbsent(toConversationDay(conversation), _ -> new ArrayList<>())
          .add(conversation);
    }
    ...
    for (var groupedEntry : groupedConversations.entrySet()) {
      entries.add(
          new TimelineDividerEntry(formatDayLabel(groupedEntry.getKey()), "divider-" + index++));
