...
public class ChatUiController implements Serializable {
  ...
  RouteInitialization initializeFromRoute(
    ...
    var resolvedConversation =
        conversationService.resolveActiveConversation(
            state.clientId().peek(), requestedConversationId);

    state.activeConversationId().set(resolvedConversation.activeConversationId());
    state.replaceMessages(
        resolvedConversation.messages().stream().map(MessageVm::fromStored).toList());
    ...
    if (requestedConversationParam != null
        && (requestedConversationId == null
            || !Objects.equals(
                requestedConversationId, resolvedConversation.activeConversationId()))) {
  ...
  public void startNewChat() {
    ...
    UI.getCurrent()
        .navigate(ChatView.class, QueryParameters.of(DRAFT_QUERY_PARAMETER, DRAFT_QUERY_VALUE));
  }

  public boolean submitPrompt(Runnable onResponseUpdated, Runnable onResponseFinished) {
    var prompt = state.composerText().peek();
    if (state.responseInProgress().peek()
        || state.pendingQuestionSet().peek() != null
        || prompt.isBlank()) {
      return false;
    }
          ...
          .subscribe(
              generatedTitle -> {
                conversationService.renameConversationIfTitleMatches(
                    clientId, conversationId, ensuredConversation.fallbackTitle(), generatedTitle);
                ...
                token -> {
                  ...
                  if (firstTokenReceived.compareAndSet(false, true)) {
                    responseMessage.update(
                        messageVm -> Objects.requireNonNull(messageVm).stopLoading());
                  }
  ...
  public void refreshConversationHistory() {
    ...
    state.replaceConversationHistory(
        conversationService.listConversations(state.clientId().peek()));
  }
  ...
  public void submitInteractiveQuestionResponse(StudentQuestionResponse response) {
        ...
        Mono.fromCallable(
                () ->
                    chatService.finalizeTurn(
                        turnId, clientId, conversationId, userInput, assistantResponse))
