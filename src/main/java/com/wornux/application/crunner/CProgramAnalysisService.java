...
public class CProgramAnalysisService {
  ...
  private final CCompilerPort compilerPort;
  private final CProgramAnalysisProperties properties;
  private final Cache<CValidationCacheKey, CValidationResult> cache;

  public CProgramAnalysisService(
      CCompilerPort compilerPort, CProgramAnalysisProperties properties) {
  ...
  public CValidationResult validateSyntax(CSourceRequest request) {
    ...
    var cacheKey =
        new CValidationCacheKey(
            sourceHash, normalizedRequest.standard(), normalizedRequest.filename(), compilerPort.cacheKey());
    return cache.get(cacheKey, ignored -> compilerPort.validateSyntax(normalizedRequest, sourceHash));
  }
  ...
  private record CValidationCacheKey(
      String sourceHash, String standard, String filename, String compilerCacheKey) {}
}
