...
class DockerContainerRunner {
    ...
    private void collectLogs(String containerId, ContainerLogs logs) {
        dockerClient.logContainerCmd(containerId).withStdOut(true).withStdErr(true).withFollowStream(true).exec(logs);
    }

    private ContainerRunResult waitForExit(String containerId, Duration timeout, ContainerLogs logs)
            throws InterruptedException {
        try {
            var exitCode = dockerClient.waitContainerCmd(containerId)
                    .start()
