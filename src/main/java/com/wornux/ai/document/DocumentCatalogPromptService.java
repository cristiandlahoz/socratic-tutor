...
public class DocumentCatalogPromptService {
    ...
    public String buildInventoryPrompt(UUID clientId) {
        ...
        List<Document> documents = documentRepository
                .findByClientIdAndStatusOrderByUpdatedAtDescCreatedAtDesc(clientId, DocumentStatus.INDEXED.name());
        ...
        if (included < total) {
            builder.append("- ... ").append(total - included).append(" more indexed document(s) omitted.\n");
        }
        log.debug(
    ...
    private String formatLine(Document document) {
        var catalogTags = document.getCatalogTags() == null ? List.<String>of() : document.getCatalogTags();
        var questionExamples = document.getCatalogQuestionExamples() == null
                ? List.<String>of()
                : document.getCatalogQuestionExamples();
        String tags = catalogTags.isEmpty() ? "none" : String.join(", ", catalogTags);
        String examples =
                questionExamples.isEmpty() ? "none" : String.join("; ", questionExamples.stream().limit(2).toList());
        return "%s | %s | topic: %s | tags: %s | can answer: %s".formatted(
