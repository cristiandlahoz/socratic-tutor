...
public class EvaluationQuestionGenerationService {
  ...
  public List<GeneratedQuestion> generateQuestions(String instruction) {
    ...
    log.info("Generated {} questions from instruction ({} chars)", questionSet.questions().size(), instruction.length());
  ...
  public List<GeneratedQuestion> fromJson(String json) {
    try {
      return objectMapper.readValue(json, objectMapper.getTypeFactory().constructCollectionType(List.class, GeneratedQuestion.class));
    } catch (JsonProcessingException e) {
