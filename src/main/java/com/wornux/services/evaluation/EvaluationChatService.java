...
public class EvaluationChatService {
    ...
    private final BeanOutputConverter<ReportResponse> reportConverter = new BeanOutputConverter<>(ReportResponse.class);
    ...
    public EvaluationTurnResponse startSession(UUID runId) {
        ...
        return new EvaluationTurnResponse(TurnType.QUESTION,
    ...
    public EvaluationTurnResponse processAnswer(UUID runId, String answer) {
        ...
        session.answers.add(new AnswerRecord(lastQuestion.questionKey(), lastQuestion.questionText(), answer));
        ...
        try {
            ...
            if (session.questions.size() < MIN_QUESTIONS) {
                ...
                session.questions.add(forced);
                persistState(session, runId);
                return new EvaluationTurnResponse(TurnType.QUESTION, forced.questionText(), forced.questionKey(), null);
            }
        }
        catch (Exception e) {
    ...
    private GeneratedQuestion generateFallbackQuestion(EvaluationSession session) {
        ...
        var fallbackPrompt = promptResources.fallbackQuestion()
                .formatted(session.evaluation.getInstruction(), session.evaluation.getTitle(), history);
        ...
        log.warn(
            "All {} fallback retries exhausted for session {}; forcing generic question",
            FALLBACK_MAX_RETRIES,
            session.run.getId());
        var forcedText =
                "¿Podés explicar con tus palabras qué entendés sobre \"" + session.evaluation.getTitle() + "\"?";
        return new GeneratedQuestion(forcedText, nextKey(session));
    }

    private NextTurnResponse decideNextTurn(EvaluationSession session) {
        var history = buildConversationHistory(session);
        var antiLoopSegment = shouldInjectAntiLoop(session) ? promptResources.antiLoopBlocked() : "";
        var promptText = promptResources.adaptivePrompt()
                .formatted(
        ...
        var prompt = Prompt.builder()
                ...
                .chatOptions(
                    OllamaChatOptions.builder().temperature(0.3).format(nextTurnConverter.getJsonSchemaMap()).build())
                .build();
    ...
    private void persistState(EvaluationSession session, UUID runId) {
        ...
        evaluationService.saveAnswers(session.evaluation.getId(), serializeAnswers(session.answers));
    }
    ...
    private EvaluationTurnResponse completeEvaluation(EvaluationSession session, UUID runId) {
        try {
            ...
            sessions.remove(runId);
            return new EvaluationTurnResponse(TurnType.COMPLETE, "¡Actividad formativa completada!", null, report);
        }
        catch (Exception e) {
            sessions.remove(runId);
    ...
    private String generateReport(EvaluationSession session) {
        ...
        var reportPrompt = promptResources.reportPrompt()
                .formatted(session.evaluation.getInstruction(), session.evaluation.getTitle(), pairsBuilder.toString());

        var prompt = Prompt.builder()
                .messages(new SystemMessage(reportPrompt))
                .chatOptions(
                    OllamaChatOptions.builder().temperature(0.3).format(reportConverter.getJsonSchemaMap()).build())
                .build();
    ...
    private String serializeAnswers(List<AnswerRecord> answers) {
        ...
        for (int i = 0; i < answers.size(); i++) {
            ...
            json.append('{')
                    .append("\"questionKey\":\"")
                    .append(escapeJson(answer.questionKey()))
                    .append("\",")
                    .append("\"questionText\":\"")
                    .append(escapeJson(answer.questionText()))
                    .append("\",")
                    .append("\"answer\":\"")
                    .append(escapeJson(answer.answer()))
                    .append("\"}");
        }
