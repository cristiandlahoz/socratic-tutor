...
public class EvaluationDialog extends Div {
    ...
    public EvaluationDialog(
        ...
        var body = new VerticalLayout(title,
                titleField,
                instructionField,
                new Hr(),
    ...
    private boolean renderAnswersAsCards(String json, Div container) {
        try {
            ...
            for (int i = 0; i < answers.size(); i++) {
                ...
                card.add(badge);
                card.add(
                    buildAnswerField("Pregunta", questionText.isBlank() ? "Sin pregunta disponible." : questionText));
                card.add(
                    buildAnswerField(
                        "Respuesta del estudiante",
                        answerText.isBlank() ? "Sin respuesta registrada." : answerText));

                if (!questionKey.isBlank()) {
                    var key = new Span("ID: " + questionKey);
                    key.getStyle().set("font-size", "0.82rem").set("color", "var(--chat-text-secondary)");
                    card.add(key);
                }
