...
class TrainingAssignmentTutorServiceTest {
    ...
    @Test
    void finalReportUsesReportPromptAndModelResponse() {
        var capturedPrompt = new AtomicReference<Prompt>();
        var promptResources = promptResources("Instructions: %s\nTitle: %s\nDecision: %s\nEvidence: %s\nLimits: %s\nTranscript:\n%s");
        var service = new TrainingAssignmentTutorService(prompt -> {
            capturedPrompt.set(prompt);
            return response("{\"report\":\"## Síntesis diagnóstica\\nGood reasoning.\\n\\n## Evidencias observables\\n- Cita evidencia del transcript.\\n\\n## Fortalezas observadas\\n- Identifica el concepto central.\\n\\n## Dudas o aspectos a trabajar\\n- Falta un ejemplo concreto.\\n\\n## Limitaciones de esta evaluación\\n- Solo hubo una respuesta breve.\\n\\n## Recomendación docente\\nRetomar con una pregunta de seguimiento.\"}");
        }, promptResources);

        var report = service.finalReport(assignment(), transcript(), success());

        assertThat(capturedPrompt.get().getInstructions().getLast().getText())
                .contains("Evaluate whether the student understands pointers in C.")
                .contains("Pointers")
                .contains("evidencia suficiente y variada")
                .contains("### Pregunta 1")
    ...
    @Test
    void finalReportAppendsCanonicalTranscriptWhenModelOnlyReturnsTranscriptHeading() {
        var service = localFallbackEnabledService(modelReturning("""
                {"report":"## Síntesis diagnóstica\\nGood reasoning.\\n\\n## Evidencias observables\\n- Cita una observación concreta.\\n\\n## Fortalezas observadas\\n- Reconoce la idea general.\\n\\n## Dudas o aspectos a trabajar\\n- Falta justificar con más detalle.\\n\\n## Limitaciones de esta evaluación\\n- El transcript sigue siendo breve.\\n\\n## Recomendación docente\\nPedir un ejemplo adicional.\\n\\n## Transcripción\\nResumen narrativo sin preguntas canónicas."}
                """));
        ...
        assertThat(report)
                .contains("## Síntesis diagnóstica")
                .contains("Resumen narrativo sin preguntas canónicas.")
                .contains("## Transcripción\n\n### Pregunta 1")
                .contains("What is a pointer?")
                .contains("I am not sure.");
    }

    @Test
    void finalReportFallsBackWhenModelReturnsMalformedJson() {
        var service = localFallbackEnabledService(modelReturning("not-json"));
        ...
        assertThat(report)
                ...
                .contains("Actividad: Pointers")
                .contains("## Síntesis diagnóstica")
                .contains("## Fortalezas observadas")
                .contains("## Evidencias observables")
                .contains("## Dudas o aspectos a trabajar")
                .contains("## Limitaciones de esta evaluación")
                .contains("## Recomendación docente")
                .contains("## Transcripción")
                .contains("La evidencia recogida no alcanza para una conclusión sólida")
                .contains("I am not sure.");
    }

    @Test
    void finalReportFallsBackWhenModelReturnsTextAroundJson() {
        var service = localFallbackEnabledService(modelReturning("Here is the report: {\"report\":\"## Síntesis diagnóstica\"}"));
    ...
    @Test
    void reportDoesNotExposeInternalDecisionMetadata() {
        var promptResources = promptResources("Instructions: %s\nTitle: %s\nPrivate closure: %s\nEvidence: %s\nLimits: %s\nTranscript:\n%s");
        var capturedPrompt = new AtomicReference<Prompt>();
        var service = new TrainingAssignmentTutorService(prompt -> {
            capturedPrompt.set(prompt);
            return response("{\"report\":\"## Síntesis diagnóstica\\nReporte limpio.\\n\\n## Evidencias observables\\n- El estudiante corrigió una premisa falsa.\\n\\n## Fortalezas observadas\\n- Detectó que el bucle compila.\\n\\n## Dudas o aspectos a trabajar\\n- Falta justificar con más detalle.\\n\\n## Limitaciones de esta evaluación\\n- La interacción previa incluyó una premisa incorrecta del tutor.\\n\\n## Recomendación docente\\nRetomar con una variante realmente errónea.\\n\\n## Transcripción\\n### Pregunta 1\\n...\"}");
        }, promptResources);

        var report = service.finalReport(assignment(), transcriptWithTutorFalsePremise(), insufficientEvidence());

        assertThat(capturedPrompt.get().getInstructions().getLast().getText())
                .doesNotContain("type=")
                .doesNotContain("answerQuality=")
                .doesNotContain("coverageStatus=");
        assertThat(report)
                .doesNotContain("type=")
                .doesNotContain("answerQuality=")
                .doesNotContain("coverageStatus=")
    ...
    @Test
    void reportFlagsTutorFalsePremiseAsEvaluationLimitation() {
        var service = localFallbackEnabledService(modelReturning("not-json"));
        ...
        assertThat(report)
                .contains("## Limitaciones de esta evaluación")
                .contains("premisa incorrecta");
    }

    @Test
    void reportCreditsStudentWhenTheyCorrectlyIdentifyValidCode() {
        var service = localFallbackEnabledService(modelReturning("not-json"));

        var report = service.finalReport(assignment(), transcriptWithTutorFalsePremise(), insufficientEvidence());

        assertThat(report)
                .contains("Corrigió una premisa incorrecta")
                .contains("compila");
    }

    @Test
    void reportCreditsStudentCorrectionsUsingSharedCorrectionSignals() {
        var service = localFallbackEnabledService(modelReturning("not-json"));

        var report = service.finalReport(assignment(), transcriptWithSharedCorrectionPhrase(), insufficientEvidence());

        assertThat(report)
                .contains("Corrigió una premisa incorrecta")
                .contains("Aportó fragmentos o señales de código");
    }

    @Test
    void reportDoesNotSayNoEvidenceWhenTranscriptContainsValidCorrections() {
        var service = localFallbackEnabledService(modelReturning("not-json"));

        var report = service.finalReport(assignment(), transcriptWithTutorFalsePremise(), insufficientEvidence());

        assertThat(report)
                .doesNotContain("ausencia total de evidencia")
                .contains("evidencia recogida ofrece algunas señales útiles");
    }

    @Test
    void reportUsesTeacherFriendlyLanguage() {
        var service = localFallbackEnabledService(modelReturning("not-json"));
    ...
    @Test
    void reportDoesNotTreatProfanityAsPrimaryDiagnosticWhenTutorWasInvalid() {
        var service = localFallbackEnabledService(modelReturning("not-json"));

        var report = service.finalReport(assignment(), transcriptWithCorrectionAndFrustration(), insufficientEvidence());

        assertThat(report)
                .contains("premisa incorrecta")
                .contains("frustración")
                .doesNotContain("La frustración del estudiante es el diagnóstico principal");
    }
