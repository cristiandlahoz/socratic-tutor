...
|----|-------|--------|----------------|-------|
| SPEC-001 | RBAC Schema and Domain Model | Pending | Database, entities, repositories | Replaces the current RBAC schema. Removes resource/action/permission tables and models roles, namespaces, assignments, class membership identity, and ownership boundaries. |
| SPEC-002 | Authorization Engine, Cache, and Annotations | Pending | Security services, Caffeine, annotations | Builds the runtime authorization engine, access snapshots, cache invalidation, and annotation-based checks for service-level permissions. |
