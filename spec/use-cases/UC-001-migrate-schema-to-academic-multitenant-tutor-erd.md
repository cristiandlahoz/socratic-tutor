...
- Introducing `grounding_vector_store`.
...
The grounding model is implemented through `GROUNDING` plus the `grounding_vector_store` table, not through separate document/chunk resources.
...
```text
ingested_document       -> legacy obsolete / conceptually replaced by grounding_vector_store
document_segment        -> legacy obsolete / conceptually replaced by grounding_vector_store
vector_store            -> legacy obsolete / conceptually replaced by grounding_vector_store.embedding
document_ingestion_job  -> legacy obsolete / no target ERD equivalent
```

### Flow

1. **Migration Runner** creates `grounding_vector_store`.
2. **Database** stores class-scoped grounding rows in `grounding_vector_store`.
3. **Database** supports vector embeddings directly in `grounding_vector_store.embedding`.

### Result

```text
Grounding is now group-class scoped through vector-store metadata.
Embeddings live directly on grounding rows.
```
...
- grounding_vector_store.id UUID PRIMARY KEY
...
grounding_vector_store.embedding
...
- **On success:** The database contains the target academic multi-tenant ERD. `account`, `tenant`, `tenant_account`, roles, permissions, `legacySubject`, `academic_period`, `group_class`, `group_class_member`, `group_class_join_code`, `conversation`, `conversation_snapshot`, `grounding_vector_store`, `training_activity`, and `training_activity_assignment` exist with required relationships, constraints, indexes, foundational RBAC data, and one initial system admin account.
...
| BR-31 | `grounding_vector_store` is scoped to a group class through metadata. |
| BR-32 | `grounding_vector_store` is written by approved grounding workflows. |
| BR-33 | Embeddings are stored on `grounding_vector_store.embedding`. |
...
| BR-58 | `GROUNDING` is a valid resource because professors need to configure group-class grounding. |
...
| BR-62 | `GROUNDING_VECTOR_STORE` must not be seeded as a separate resource in this use case because it is controlled through `GROUNDING`. |
...
| BR-76 | `role`, `resource`, `action`, `permission`, `group_class_join_code`, and `conversation_snapshot` use BIGINT identity primary keys. |
...
- [ ] Stage 8 creates `grounding_vector_store`.
- [ ] Stage 8 uses `UUID PRIMARY KEY` for `grounding_vector_store.id`.
- [ ] Stage 8 stores embeddings on `grounding_vector_store.embedding`.
...
- [ ] Stage 3 does not seed `GROUNDING_VECTOR_STORE` as a separate authorization resource.
...
| Grounding model | `grounding_vector_store`. |
