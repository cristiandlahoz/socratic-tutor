...
Domain entities use UUID primary keys except `role.id`, `resource.id`, `action.id`, `permission.id`, `group_class_join_code.id`, and `conversation_snapshot.id`, which use `BIGINT GENERATED BY DEFAULT AS IDENTITY`.
